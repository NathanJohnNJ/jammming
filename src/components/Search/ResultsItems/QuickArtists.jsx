      ...
      <NavLink to="/info" state={artist} className="flex w-full justify-self-center rounded-md">
        <div className="w-full h-fit ml-3 mr-3 pl-4 py-0.5  text-neutral-800 font-light flex flex-col items-start justify-center text-xs rounded-md">
          <div className="flex flex-col items-centre justify-evenly w-[80%]">
            <p className="text-lg font-semibold text-left mt-4 mb-2">{artist.name}</p>
            <div className="flex flex-col items-start">
              <p className="text-[0.9em] font-semibold mr-4 flex"><IoPeopleCircleOutline className="text-[1.1rem] mb-2 mx-1" /> Followers: <span className="font-extrabold">{artist.followers.total}</span></p>
              <p className="text-[0.9em] font-semibold flex"><TbChartBarPopular className="text-[1.1rem] mx-1 mb-2" /> Popularity: <span className="font-extrabold">{artist.popularity}%</span></p>
            </div>
          </div>
          { artist.genres != [] &&
          <p className="overflow-x-scroll text-[0.9em] flex items-center"><span className="font-medium">Genres</span>:&nbsp;{artist.genres.map((genre,i) => {
